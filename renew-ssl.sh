...
REQUIRED_CA="$CERT_FOLDER/$DOMAIN_NAME.ca-bundle"

if [ -f "$REQUIRED_KEY" ] && [ -f "$REQUIRED_CRT" ] && [ -f "$REQUIRED_INT" ] && [ -f "$REQUIRED_CA" ]; then
    ...
    echo "  - intermediate.crt (Intermediate Certificate Chain)"
    echo "  - $DOMAIN_NAME.ca-bundle (CA Bundle - same as intermediate)"
    echo ""
    ...
    echo "  - $DOMAIN_NAME.crt (Certificate)"
    echo "  - intermediate.crt (Intermediate Certificate Chain)"
    echo "  - $DOMAIN_NAME.ca-bundle (CA Bundle)"
    echo ""
